...
4. **Generator:** `poolsFor(v, breakWeek)` builds Day/Night/Late pools by slider position, widens thin pools to ≥3 from neighbouring colours; `generate(v, hasLate, sigCount, seed, breakWeek, doses, days)` runs over the active dose days only (`act`), places the reserve quota first (`sigDays` picks positions in the dose-day sequence, never neighbours, wraps; quota clamped to floor(n/2)), then random fill with `validWeek(w, hasLate, doses, act)`. Off days are `null` entries in `week[7]`; one-dose days have `night: null`. Every consumer of `p.week[d]` must guard null (table, today card, burn, usage, journal, PDF). Harness: scratchpad `gen-test.js` (extracts catalog+generator+helpers from the inline script, 1,944 combinations, all pass as of 2026-09-17). If Late can't be filled the gen handler falls back to two-dose days with an alert. Tested with a Node harness across strain sets (`gen-test.js` pattern in scratchpad; extract `catalog` + `generator` sections, stub `state`, `effLate`).
...
7. **UI:** two tabs (`view-week` / `view-stock` body class); every panel = gist + `<details class="more" data-more=...>`; today card with check circles (journal only, no stock effect); standing items list; settings/first-run panel (`#setupPanel`, shown when `!state.setup` or toggled); backup/restore. First run does NOT auto-generate a week and does NOT prefill amounts: the Welcome panel asks doses a day (1/2/3), dose days (chips) and capsules per dose (blank, required); Done then opens the controls fold. New profiles default to two doses, every day, servings 3/3/2/2 (only reached via '+ add a person'; editable under the slider).
...
6. **Liability posture (2026-09-17):** the app prescribes nothing (amounts, dose count and dose days are entered by the person, blank on first run); first run requires a 21+/legal-where-you-live confirmation (`state.confirmed` = date); copy avoids "rules", "guarantees", "tolerance" and dose-spacing instructions (app rules block, PDF, journal trend, landing page); the landing page has Terms of Use (no medical advice, no warranty, limitation of liability, purchases on the store's terms, affiliate note) and Privacy sections; strain descriptions are labelled as the store's copy in the rules block, the PDF footer and the landing page. Keep it that way: any new copy that tells people what to take, how much, or how often is a regression. Joshua still intends to read the affiliate program's marketing terms and get a lawyer's look if it takes off.
7. Nothing else pending beyond public launch.
